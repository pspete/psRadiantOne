...
function Import-R1JdbcDriver {
	...
	Process {
		...
		$URI = Resolve-R1ServiceUrl -Service Catalog -Path 'jdbc_drivers/upload'

		$UploadFile = Get-Item -Path $Path

		$Form = ConvertTo-MultipartFormData -Field @{ file = $UploadFile }

		if ($PSCmdlet.ShouldProcess($UploadFile.Name, 'Upload JDBC Driver')) {
