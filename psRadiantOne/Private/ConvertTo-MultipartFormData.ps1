function ConvertTo-MultipartFormData {
	...
	.DESCRIPTION
	Constructs a raw multipart/form-data byte payload and boundary marker, for use as the Body of
	an Invoke-R1RestMethod call together with an explicit -ContentType of the returned value.
	...
	Get-Item) are sent as file parts, with a Content-Type derived from the file's extension; all
	...
	.EXAMPLE
	$Form = ConvertTo-MultipartFormData -Field @{ file = (Get-Item .\driver.jar) }
