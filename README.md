...
- **Prior to a Version 1.0.0 release**:
  ...
  - Many commands are built from the published API definition but have not yet been exercised against a live deployment - see [Help Us Test](#help-us-test) below, your feedback genuinely shapes what ships next.
...
_psRadiantOne_ currently ships 300+ commands, grouped into the areas below. The full list is not reproduced command-by-command here - instead, once the module is imported:
...
Prior to a 1.0.0 release, roughly half of the commands have not yet been exercised against a live deployment: their behaviour rests on the vendor's published API definition alone. Each of those commands says so in the `NOTES` section of its help.
