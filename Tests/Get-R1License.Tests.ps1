...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Mock Invoke-R1RestMethod -MockWith { [pscustomobject]@{ 'type' = 'FULL'; 'product' = 'FID'; 'valid' = $true } }

			$response = Get-R1License
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/settings-service/license')
		...
		Context 'Output' {
			...
			It 'has expected typename' {
				...
				$response.psobject.TypeNames[0] | Should -Be 'psRadiantOne.LicenseInfo'
