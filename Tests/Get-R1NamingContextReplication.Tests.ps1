...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			$Expected = 'true' | ConvertFrom-Json

			Mock Invoke-R1RestMethod -MockWith { $Expected }

			$response = Get-R1NamingContextReplication -dn 'o=companydirectory'
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/directory-namespace-service/naming_contexts/o%3Dcompanydirectory/replication')
		...
		Context 'Output' {
			...
			It 'returns nothing when the API returns nothing' {
				...
				Get-R1NamingContextReplication -dn 'o=companydirectory' | Should -BeNullOrEmpty
