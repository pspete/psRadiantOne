	...
	InModuleScope 'psRadiantOne' {
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/settings-service/features')
			...
			It 'sends request to expected endpoint when a flag is named' {

				Get-R1Feature -flagId featureA

				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {

					$URI -eq 'https://radiantone.company.com/settings-service/features/featureA'
		...
		Context 'Output' {
			...
			It 'has expected typename' {
				...
				$response[0].psobject.TypeNames[0] | Should -Be 'psRadiantOne.FeatureFlag'
