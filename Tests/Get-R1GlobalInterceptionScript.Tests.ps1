	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Mock Invoke-R1RestMethod -MockWith {
				[pscustomobject]@{
					'classname'      = $null
					'filename'       = 'globalIntercept.java'
					'scriptContents' = 'public class GlobalIntercept { }'
				}
			}

			$response = Get-R1GlobalInterceptionScript
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/directory-namespace-service/interception_scripts/global')
