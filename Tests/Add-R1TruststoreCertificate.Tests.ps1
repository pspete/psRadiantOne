...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Set-Content -Path $CertFile -Value 'certdata'

			Add-R1TruststoreCertificate -alias 'SomeAlias' -Path $CertFile -Confirm:$false
		...
		Context 'Input' {
			...
			It 'includes the alias and the file in the form' {

				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {

					$Decoded = [System.Text.Encoding]::UTF8.GetString($Body)
					($Decoded -match 'name="alias"') -and ($Decoded -match 'name="file"; filename="cert.cer"')

				} -Times 1 -Exactly -Scope It

			}

			It 'rejects a file which does not exist' {

				{ Add-R1TruststoreCertificate -alias 'SomeAlias' -Path (Join-Path $TestDrive 'missing.cer') -Confirm:$false } |
					Should -Throw
