		...
		BeforeEach {
			...
			Mock Invoke-R1RestMethod -MockWith {
				[pscustomobject]@{
					...
					'claimsMapper'       = [pscustomobject]@{ 'claimsExpressionList' = @('uid=$sub') }
				}
			}

			Set-R1TokenValidator -name 'SomeValidator' -enabled $false -Confirm:$false
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($Method -eq 'PUT') -and ($URI -eq 'https://radiantone.company.com/settings-service/external_token_validators/SomeValidator')
			...
			It 'retrieves the validator before updating it' {
			...
			It 'preserves properties which were not specified' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($d.jsonWebKeySetUri -eq 'https://idp.example.com/jwks') -and ($d.jwtValidationClock -eq 30)

				} -Times 1 -Exactly -Scope It

			}

			It 'preserves the existing claimsMapper when no expressions are specified' {
