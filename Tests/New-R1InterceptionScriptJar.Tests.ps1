	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Mock Invoke-R1RestMethod -MockWith {
				[pscustomobject]@{
					'exitCode' = 0
					'messages' = 'BUILD SUCCESSFUL'
				}
			}

			$response = New-R1InterceptionScriptJar -Confirm:$false
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/directory-namespace-service/interception_scripts/build_jar')
		...
		Context 'Output' {
			...
			It 'has expected typename' {
				...
				$response.psobject.TypeNames[0] | Should -Be 'psRadiantOne.InterceptionJarBuildResults'
