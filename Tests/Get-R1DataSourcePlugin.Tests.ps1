	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Mock Invoke-R1RestMethod -MockWith {
				@([pscustomobject]@{ 'name' = 'acs'; 'version' = '1.0' })
			}

			$response = Get-R1DataSourcePlugin
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/data-catalog-service/meta/plugins')
		...
		Context 'Output' {
			...
			It 'has expected typename' {
				...
				@($response)[0].psobject.TypeNames[0] | Should -Be 'psRadiantOne.DataSourcePlugin'
