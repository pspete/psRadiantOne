...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Remove-R1TruststoreCertificate -alias 'SomeAlias' -Confirm:$false
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/settings-service/client_certificate_truststore/SomeAlias')
