...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			$Expected = '{"name":"backup1"}' | ConvertFrom-Json

			Mock Invoke-R1RestMethod -MockWith { $Expected }

			$response = Get-R1StoreBackup -dn 'o=companydirectory'
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/directory-namespace-service/naming_contexts/o%3Dcompanydirectory/store/list_backups')
		...
		Context 'Output' {
			...
			It 'has expected typename' {
				...
				$response.psobject.TypeNames[0] | Should -Be 'psRadiantOne.StoreBackup'
			...
			It 'returns nothing when the API returns nothing' {
				...
				Get-R1StoreBackup -dn 'o=companydirectory' | Should -BeNullOrEmpty
