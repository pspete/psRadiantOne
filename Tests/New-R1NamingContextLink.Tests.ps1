...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			New-R1NamingContextLink -dn 'o=vds' -linkType 'STANDARD_LINK' -existingView 'adap' -Confirm:$false
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/directory-namespace-service/naming_contexts/o%3Dvds/add_link')
			...
			It 'sends the specified values' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($Decoded.linkType -eq 'STANDARD_LINK') -and ($Decoded.existingView -eq 'adap')
