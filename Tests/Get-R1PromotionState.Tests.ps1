	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Mock Invoke-R1RestMethod -MockWith {
				[pscustomobject]@{ 'state' = 'INACTIVE' }
			}

			$response = Get-R1PromotionState
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/settings-service/configuration/state/auto')
		...
		Context 'Output' {
			...
			It 'has expected typename' {
				...
				@($response)[0].psobject.TypeNames[0] | Should -Be 'psRadiantOne.PromotionState'
