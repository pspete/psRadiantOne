		...
		BeforeEach {
			...
			Mock Invoke-R1RestMethod -MockWith {
				[pscustomobject]@{
					...
					'preOperationInterceptOn'     = @('SEARCH', 'BIND')
					'postOperationInterceptAfter' = @('ADD')
				}
			}

			Set-R1GlobalInterceptionSetting -preOperationInterceptOn 'DELETE' -Confirm:$false
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($Method -eq 'PUT') -and ($URI -eq 'https://radiantone.company.com/directory-namespace-service/global_interception_settings')
			...
			It 'sends the specified value' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					@(($Body | ConvertFrom-Json).preOperationInterceptOn) -contains 'DELETE'
			...
			It 'replaces, rather than adds to, the specified collection' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					if ($Method -ne 'PUT') { return $false }
					@(($Body | ConvertFrom-Json).preOperationInterceptOn).Count -eq 1
			...
			It 'preserves settings which were not specified' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					if ($Method -ne 'PUT') { return $false }
					@(($Body | ConvertFrom-Json).postOperationInterceptAfter) -contains 'ADD'
