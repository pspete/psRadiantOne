...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			$UploadFile = Join-Path $TestDrive 'MyInterception.java'
			Set-Content -Path $UploadFile -Value 'public class MyInterception { }' -Encoding Ascii

			Import-R1InterceptionScript -Path $UploadFile -Confirm:$false
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/directory-namespace-service/interception_scripts')
			...
			It 'sends the file as the file form field' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					$Decoded -match 'Content-Disposition: form-data; name="file"; filename="MyInterception.java"'
			...
			It 'declares the content type of the file' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					$Decoded = [System.Text.Encoding]::UTF8.GetString($Body)
					$Decoded -match 'Content-Type: text/x-java-source'
			...
			It 'sends the overwrite flag using the lowercase json spelling' {
				...
				$File2 = Join-Path $TestDrive 'second-MyInterception.java'
				Set-Content -Path $File2 -Value 'public class MyInterception { }' -Encoding Ascii

				Import-R1InterceptionScript -Path $File2 -isOverwrite $true -Confirm:$false
