...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Update-R1AttributeEncryptionKey -cipher AES256 -secretKey ('s3cret' | ConvertTo-SecureString -AsPlainText -Force) -Confirm:$false
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/settings-service/attribute_encryption/update_key')
			...
			It 'sends expected request body' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($d.cipher -eq 'AES256') -and ($d.secretKey -eq 's3cret')
