...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint when name specified' {

				Get-R1TokenValidator -name 'Some Validator'

				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {

					$URI -eq 'https://radiantone.company.com/settings-service/external_token_validators/Some%20Validator'
		...
		Context 'Output' {
			...
			It 'has expected typename' {
				...
				$response.psobject.TypeNames[0] | Should -Be 'psRadiantOne.ExternalTokenValidator'
