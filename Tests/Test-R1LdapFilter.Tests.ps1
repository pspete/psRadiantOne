...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			$response = Test-R1LdapFilter -ldapFilter '(objectClass=*)'
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					$URI -match '^https://radiantone\.company\.com/directory-namespace-service/naming_context_utils/validate_ldap_filter\?'
			...
			It 'sends the value as a query parameter' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter { $URI -match 'ldapFilter=' } -Times 1 -Exactly -Scope It
		...
		Context 'Output' {
			...
			It 'returns false when the api reports the value invalid' {
				...
				Test-R1LdapFilter -ldapFilter '(objectClass=*)' | Should -BeFalse
