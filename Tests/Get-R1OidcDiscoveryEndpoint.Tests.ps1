...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Mock Invoke-R1RestMethod -MockWith { @('https://idp.example.com/.well-known/openid-configuration') }

			$response = Get-R1OidcDiscoveryEndpoint
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/settings-service/oidc_providers/discovery_endpoints')
