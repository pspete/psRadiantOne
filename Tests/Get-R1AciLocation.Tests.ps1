...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Mock Invoke-R1RestMethod -MockWith { @('cn=config', 'dc=example,dc=com') }

			$response = Get-R1AciLocation
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/settings-service/access_control/acis/locations')
		...
		Context 'Output' {
			...
			It 'returns the locations' {

				$response | Should -Contain 'cn=config'
