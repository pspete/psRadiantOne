	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Mock Invoke-R1RestMethod -MockWith {
				@([pscustomobject]@{ 'name' = 'postgres.jar'; 'classNames' = @('org.postgresql.Driver') })
			}

			$response = Get-R1JdbcDriverFile
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/data-catalog-service/jdbc_drivers/files')
		...
		Context 'Output' {
			...
			It 'has expected typename' {
				...
				@($response)[0].psobject.TypeNames[0] | Should -Be 'psRadiantOne.DriverFileClassName'
