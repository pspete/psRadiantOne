...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			$response = Get-R1AccessControlSetting
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/settings-service/access_control')
		...
		Context 'Output' {
			...
			It 'has expected typename' {
				...
				$response.psobject.TypeNames[0] | Should -Be 'psRadiantOne.AccessControlSetting'
