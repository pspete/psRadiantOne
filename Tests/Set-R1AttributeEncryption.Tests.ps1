		...
		BeforeEach {
			...
			Mock Invoke-R1RestMethod -MockWith {
				[pscustomobject]@{
					...
					'currentHdapCipher'  = 'AES256'
					'hdapAttrKeyExists'  = $true
					'currentLdifzCipher' = 'AES128'
					'ldifzKeyExists'     = $false
					'secureLdifExport'   = $true
					'availableCiphers'   = @('AES256', 'AES128', 'DES3')
				}
			}

			Set-R1AttributeEncryption -secureLdifExport $false -Confirm:$false
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($Method -eq 'PUT') -and ($URI -eq 'https://radiantone.company.com/settings-service/attribute_encryption')
			...
			It 'preserves settings which were not specified' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($Body | ConvertFrom-Json).currentHdapCipher -eq 'AES256'

				} -Times 1 -Exactly -Scope It

			}

			It 'does not send the state properties the api maintains' {

				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {

					if ($Method -ne 'PUT') { return $false }
					$d = $Body | ConvertFrom-Json
					($null -eq $d.hdapAttrKeyExists) -and ($null -eq $d.availableCiphers)
