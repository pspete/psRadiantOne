	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Mock Invoke-R1RestMethod -MockWith {
				@('Directory String syntax', 'Integer syntax - integral number', 'JPEG')
			}

			$response = Get-R1DirectoryAttributeSyntax
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/directory-schema-service/attributes/attr_syntax')
