...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			$response = Get-R1AttributeEncryptionKmsSetting
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/settings-service/attribute_encryption/aws_kms')
		...
		Context 'Output' {
			...
			It 'reports whether the credentials exist, not their values' {

				$response.accessKeySecretExists | Should -BeTrue
				$response.psobject.Properties.Name | Should -Not -Contain 'accessKeySecret'
