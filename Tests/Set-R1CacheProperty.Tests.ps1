#InModuleScope is resolved during Pester's discovery phase, so the module must be imported here
#rather than from BeforeAll, which does not run until the later run phase.

#Get Current Directory
$Here = Split-Path -Parent $PSCommandPath

#Module Name
$ModuleName = 'psRadiantOne'

#Resolve Path to Module Directory
$ModulePath = Resolve-Path "$Here\..\$ModuleName"

#Define Path to Module Manifest
$ManifestPath = Join-Path "$ModulePath" "$ModuleName.psd1"

if ( -not (Get-Module -Name $ModuleName -All)) {

	Import-Module -Name "$ManifestPath" -ArgumentList $true -Force -ErrorAction Stop

}

Describe $($PSCommandPath -Replace '.Tests.ps1') {

	InModuleScope 'psRadiantOne' {

		BeforeEach {

			$psRadiantOneSession = [ordered]@{
				BaseURI            = 'https://radiantone.company.com'
				User               = 'uid=testuser,ou=globalusers,cn=config'
				Token              = 'SomeToken'
				TokenExpiry        = $null
				Privileges         = $null
				Organization       = $null
				Version            = $null
				WebSession         = $null
				StartTime          = $null
				ElapsedTime        = $null
				LastCommand        = $null
				LastCommandTime    = $null
				LastCommandResults = $null
				LastError          = $null
				LastErrorTime      = $null
			}
			New-Variable -Name psRadiantOneSession -Value $psRadiantOneSession -Scope Script -Force

			Mock Invoke-R1RestMethod -MockWith {
				[pscustomobject]@{
					'isActive' = $true
					'isConfigured' = $true
					'isInitialized' = $true
					'startingSuffix' = 'o=cache'
					'internalSuffix' = 'o=cache'
					'isFullTextSearch' = $false
					'storageLocation' = $null
					'isUseCacheForAuth' = $true
					'isLocalBindOnly' = $false
					'isDelegateOnFailure' = $false
					'isEnablePasswordPolicyEnforcement' = $false
					'isPasswordWriteBack' = $false
					'isOptimizeLinkedAttributes' = $false
					'caseSensitiveAttributesCompare' = $false
					'caseSensitiveAttributes' = @()
					'nonIndexedAttributes' = @('userpassword')
					'sortedAttributes' = @()
					'encryptedAttributes' = @()
					'extensionAttributes' = @()
					'invariantAttribute' = $null
					'isInterClusterReplication' = $false
					'isEnsurePushMode' = $false
					'pushModeDataSources' = @()
					'replicationExcludedAttributes' = @()
					'isAcceptChangesFromReplicas' = $false
					'updatableAttributesFromReplicas' = @()
					'changeLogEnabled' = $false
					'asyncIndexingEnabled' = $false
				}
			}

			Set-R1CacheProperty -dn 'o=cache' -isActive $false -Confirm:$false

		}

		Context 'Input' {

			It 'sends request to expected endpoint' {

				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {

					($Method -eq 'PUT') -and ($URI -eq 'https://radiantone.company.com/directory-namespace-service/caches/o%3Dcache/properties')

				} -Times 1 -Exactly -Scope It

			}

			It 'retrieves the current properties before updating them' {

				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter { $Method -eq 'GET' } -Times 1 -Exactly -Scope It

			}

			It 'sends the specified value' {

				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {

					if ($Method -ne 'PUT') { return $false }
					($Body | ConvertFrom-Json).isActive -eq $false

				} -Times 1 -Exactly -Scope It

			}

			It 'preserves properties which were not specified' {

				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {

					if ($Method -ne 'PUT') { return $false }
					$Decoded = $Body | ConvertFrom-Json
					($Decoded.isUseCacheForAuth -eq $true) -and ($Decoded.startingSuffix -eq 'o=cache') -and (@($Decoded.nonIndexedAttributes)[0] -eq 'userpassword')

				} -Times 1 -Exactly -Scope It

			}

			It 'sends a single attribute as an array' {

				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {

					if ($Method -ne 'PUT') { return $false }
					$Body -match '"nonIndexedAttributes"\s*:\s*\[\s*"userpassword"\s*\]'

				} -Times 1 -Exactly -Scope It

			}

			It 'sends empty collections as empty arrays' {

				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {

					if ($Method -ne 'PUT') { return $false }
					$Body -match '"sortedAttributes"\s*:\s*\[\s*\]'

				} -Times 1 -Exactly -Scope It

			}

			It 'does not send the node' {

				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {

					if ($Method -ne 'PUT') { return $false }
					-not ($Body -match '"node"')

				} -Times 1 -Exactly -Scope It

			}

		}

	}

}
