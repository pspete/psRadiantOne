...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Set-R1CustomLimit -limits @{ dnType = 'sub-tree'; dn = 'dc=example,dc=com'; maxConnection = 10 } -Confirm:$false
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/settings-service/limits/custom')
			...
			It 'sends the limits as an array' {
			...
			It 'sends an empty array when given none' {
				...
				Set-R1CustomLimit -limits @() -Confirm:$false
