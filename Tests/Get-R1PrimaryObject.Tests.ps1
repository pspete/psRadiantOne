...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			$Expected = '"inetOrgPerson"' | ConvertFrom-Json

			Mock Invoke-R1RestMethod -MockWith { $Expected }

			$response = Get-R1PrimaryObject -dn 'o=companyprofiles'
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/directory-namespace-service/naming_contexts/o%3Dcompanyprofiles/object_builder/primary_objects')
		...
		Context 'Output' {
			...
			It 'returns nothing when the API returns nothing' {
				...
				Get-R1PrimaryObject -dn 'o=companyprofiles' | Should -BeNullOrEmpty
