...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			$response = Get-R1SchemaFullObject -schemaName 'default'
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/data-catalog-service/schemas/default/full_obj')
		...
		Context 'Output' {
			...
			It 'has expected typename' {
				...
				@($response)[0].psobject.TypeNames[0] | Should -Be 'psRadiantOne.SchemaComplete'
