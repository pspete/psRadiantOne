...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Mock Invoke-R1RestMethod -MockWith { [pscustomobject]@{ 'configurationName' = 'SomeProvider' } }

			$response = Get-R1OidcLoginInfo
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/settings-service/oidc_providers/login_info')
		...
		Context 'Output' {
			...
			It 'has expected typename' {
				...
				$response.psobject.TypeNames[0] | Should -Be 'psRadiantOne.OidcLoginInfo'
