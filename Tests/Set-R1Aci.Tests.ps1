...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Set-R1Aci -aciId 12345 -baseDn 'cn=config' -permsType DENY -Confirm:$false
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($Method -eq 'PUT') -and ($URI -eq 'https://radiantone.company.com/settings-service/access_control/acis/12345?baseDn=cn%3Dconfig')
			...
			It 'retrieves the aci before updating it' {
			...
			It 'sends the specified value' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($Body | ConvertFrom-Json).permsType -eq 'DENY'
			...
			It 'preserves properties which were not specified' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($Decoded.name -eq 'SomeName') -and (@($Decoded.selectedOperations).Count -eq 2)
