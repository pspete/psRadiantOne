...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Mock Invoke-R1RestMethod -MockWith { @('splunk', 'elastic') }

			$response = Get-R1PipelineConnectorType -pipelineId 'pipe-1'
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/settings-service/identity_observability/pipe-1/connector_types')
		...
		Context 'Output' {
			...
			It 'returns the available types' {

				$response | Should -Contain 'splunk'
