...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Mock Invoke-R1RestMethod -MockWith { @('PBKDF2', 'SSHA') }

			$response = Get-R1PasswordEncryption
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/settings-service/password_policies/password_encryption')
		...
		Context 'Output' {
			...
			It 'returns the algorithms' {

				$response | Should -Contain 'PBKDF2'
