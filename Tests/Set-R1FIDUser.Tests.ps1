...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		Context 'Input' {
			...
			It 'omits password when not specified' {
			...
			It 'sends password when specified' {

				Set-R1FIDUser -username john_smith -active $true -password ('N3wP@ss' | ConvertTo-SecureString -AsPlainText -Force) -Confirm:$false
