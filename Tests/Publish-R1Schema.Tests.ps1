...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Publish-R1Schema -schemaName 'default' -Confirm:$false
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/data-catalog-service/schemas/published')
			...
			It 'collects names supplied through the pipeline into one request' {
				...
				'first', 'second', 'third' | Publish-R1Schema -Confirm:$false
