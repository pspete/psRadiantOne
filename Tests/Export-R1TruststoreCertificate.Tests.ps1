...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Export-R1TruststoreCertificate -alias 'SomeAlias' -OutFile (Join-Path $TestDrive 'out.cer')
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($URI -eq 'https://radiantone.company.com/settings-service/client_certificate_truststore/SomeAlias/export')
			...
			It 'passes the output file through to the request' {

				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {

					$OutFile -match 'out\.cer$'
