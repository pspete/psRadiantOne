...
Describe $($PSCommandPath -Replace '.Tests.ps1') {
	...
	InModuleScope 'psRadiantOne' {
		...
		BeforeEach {
			...
			Set-R1RestClientAccess -maxThreads 500 -Confirm:$false
		...
		Context 'Input' {
			...
			It 'sends request to expected endpoint' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($Method -eq 'PUT') -and ($URI -eq 'https://radiantone.company.com/settings-service/client_access/rest')
			...
			It 'sends the specified value' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					($Body | ConvertFrom-Json).maxThreads -eq 500
			...
			It 'preserves settings which were not specified' {
				...
				Should -Invoke -CommandName Invoke-R1RestMethod -ParameterFilter {
					...
					if ($Method -ne 'PUT') { return $false }
					@(($Body | ConvertFrom-Json).tokenTimeout) -contains 360000
